...
@NoArgsConstructor
@ApiModel
public class ApiWithdrawConfigEditParam {
    @NotNull
    ...
    @NotNull
    @ApiModelProperty(value = "编码，每种编码都有不同的处理类", required = true, position = 3)
    private ApiWithdrawConfigCodeEnum code;

    @NotNull
    @ApiModelProperty(value = "适用提现方式", required = true, position = 4)
    private WithdrawConfigSourceEnum withdrawConfigSource;

    @ApiModelProperty(value = "三方配置，密钥，URL什么的，JSON格式", required = true, position = 5)
    private String thirdConfig;

    @ApiModelProperty(value = "三方回调IP白名单", required = true, position = 6)
    private String thirdCallbackWhitelistIp;

    @ApiModelProperty(value = "排序号，越小排越前面", required = true, position = 7)
    private Integer sort;

    @NotNull
    @ApiModelProperty(value = "启用/禁用", required = true, position = 8)
